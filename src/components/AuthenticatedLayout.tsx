...
// Extended Job interface for ADF hierarchy
...
export function AuthenticatedLayout() {
  ...
  const [expandedDataFactories, setExpandedDataFactories] = useState<Set<string>>(new Set());
  const [jobs, setJobs] = useState<ADFJob[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchJobs = async () => {
      try {
        ...
        setJobs([...glueJobItems, ...resourceGroupItems, ...mockJobs]);
      } catch (error: any) {
        toast({
          variant: "destructive",
          title: "Error",
  ...
  const handleResourceGroupClick = async (resourceGroup: ADFJob) => {
    ...
    if (isExpanded) {
      // Collapse: remove data factories and pipelines for this resource group
      ...
      // Expand: fetch and add data factories
      try {
  ...
  const handleDataFactoryClick = async (dataFactory: ADFJob) => {
    ...
    if (isExpanded) {
      // Collapse: remove pipelines for this data factory
      ...
      // Expand: fetch and add pipelines
      try {
  ...
  const handleJobClick = (job: ADFJob) => {
    if (job.isResourceGroup) {
      // Resource Group - only expand/collapse, don't open dialog
      handleResourceGroupClick(job);
    } else if (job.isDataFactory) {
      // Data Factory - only expand/collapse, don't open dialog
      handleDataFactoryClick(job);
    } else if (job.isPipeline) {
      // Pipeline - open dialog to configure
      ...
      // Glue job - open dialog to configure
      localStorage.setItem("selectedJob", job.name);
      ...
      // Lambda, Batch, Airflow - open dialog to configure
      localStorage.setItem("selectedJob", job.name);
        ...
        <CardContent className="p-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {(job.isResourceGroup || job.isDataFactory) && (
