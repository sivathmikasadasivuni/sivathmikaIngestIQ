...
export default function ViewJobDialog({ job, open, onOpenChange }: ViewJobDialogProps) {
  ...
  // Reorder stages: move step 5 to step 3, remove old step 3 and old step 5
  const reorderedStages = job.stages ? (() => {
    const stages = [...job.stages];
    if (stages.length >= 5) {
      const step5 = stages[4]; // Get step 5 (index 4)
      stages.splice(4, 1); // Remove step 5 from its current position
      stages.splice(2, 1, step5); // Replace step 3 (index 2) with step 5
    }
    return stages;
  })() : [];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {/* Job Steps */}
          {reorderedStages.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold mb-4">Job Steps ({reorderedStages.length})</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
