...
// interface RunStepFunctionPayload {
//   job_id: string;
//   token: string;
//   jobType?: string;
//   glueName?: string;
...
//       // Continue polling if still "Running" and not immediate
...
// try {
//     const token = localStorage.getItem("authToken"); // Use localStorage directly
//     if (!token) {
//       toast({
//         variant: "destructive",
//         title: "Error",
...
//   title="Run Job"
//   disabled={job.status === "Running"} // Disable when Running
// >
...
// the above is my full integrated code
//-----------------------------------------------------------------------------------------------------------//
...
// Category filter options - dynamically generated from jobs
const categoryFilterOptions = ["All", "Glue", "ADF", "Lambda", "Airflow", "Batch"]
...
export default function Jobs() {
  ...
  useEffect(() => {
    ...
    if (chartCategoryFilter !== "All") {
      filtered = filtered.filter((job) => job.category === chartCategoryFilter)
    }
  ...
  const handleRunJob = async (job: LocalJob) => {
    ...
    try {
      ...
      const payload: RunStepFunctionRequest = {
        job_id: job.jobId,
        token: token,
        ...(job.job_type && { jobType: job.job_type }), // Use job_type for consistency
        ...(job.glueName && { glueName: job.glueName }),
      }
  ...
  const handleViewJob = async (job: LocalJob) => {
    ...
    try {
      const response: GetJobDetailsResponse = await getJobDetails(job.jobId)
      if (response.success && response.job) {
        const category = response.job.job_type
          ? response.job.job_type.charAt(0).toUpperCase() + response.job.job_type.slice(1).toLowerCase()
          : "Unknown"
        ...
        const detailedJob: LocalJob = {
          ...
          steps: response.job.steps || job.steps,
          business_logic_rules: response.job.business_logic_rules || job.business_logic_rules || {},
          job_type: response.job.job_type || job.job_type,
          glueName: response.job.glueName || job.glueName,
        }
        setViewingJob(detailedJob)
      } else {
    ...
    try {
      ...
      if (response.success && response.job) {
        ...
        const detailedJob: LocalJob = {
          ...
          steps: response.job.steps || job.steps,
          business_logic_rules: response.job.business_logic_rules || job.business_logic_rules || {},
          job_type: response.job.job_type || job.job_type, // Store job_type
          glueName: response.job.glueName || job.glueName,
        }
        setEditingJob(detailedJob)
      } else {
